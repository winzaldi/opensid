<?php class Keluar_model extends CI_Model {
	...
	public function tambah_tte($id=0)
	{
		$sql = "UPDATE log_surat SET tte = 'Y' WHERE id = ?";
		$outp = $this->db->query($sql, $id);
		if($outp){
			$sql = "insert into tanda_tangan_surat(`id_format_surat`, `id_pend`, `id_pamong`, `id_user`, `tanggal`, 
				   `bulan`, `tahun`, `no_surat`, `nama_surat`, `lampiran`, `nik_non_warga`, `nama_non_warga`, 
				    ...
				    `nama_surat`, `lampiran`, `nik_non_warga`, `nama_non_warga`, `keterangan`,'0',nama_surat 
				    from log_surat  where id = ?";
			$outp = $this->db->query($sql, $id);
